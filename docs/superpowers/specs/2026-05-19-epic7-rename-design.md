...
| Old | New | Layer |
|-----|-----|-------|
| `llm-api` (k8s resource/label names) | `llm-api` | k8s YAMLs |
| `llm-api-secrets` | `llm-api-secrets` | k8s Secret |
| `llm-api-db` | `llm-api-db` | k8s StatefulSet + Service |
| `llm-api-db-secret` | `llm-api-db-secret` | k8s Secret |
| `llm-api` | `llm-api` | `pyproject.toml` |
| `llm-api:latest` | `llm-api:latest` | `docker-compose.yml` |
| `aipet` (docker-compose service name) | `llm-api` | `docker-compose.yml` |
| `llm-api-training` (Temporal task queue) | `llm-api-training` | Python source |
| `model.gguf` | `model.gguf` | Python source + k8s env var |
| `~/llm-api` (SSH/remote work dir) | `~/llm-api` | SSH adapter + remote compute |
| `infra/k8s/llm-api/` | `infra/k8s/llm-api/` | directory |
| ECR repo `llm-api` | `llm-api` | Terraform + `deploy.yml` |
| ECR repo `llm-api-temporal-ui` | `llm-api-temporal-ui` | `deploy.yml` env var |
...
1. `git mv infra/k8s/llm-api infra/k8s/llm-api` — rename the k8s directory.
...
- Update `ECR_REPOSITORY` in `.github/workflows/deploy.yml` from `llm-api` to `llm-api`.
- Update `ECR_TEMPORAL_UI_REPOSITORY` from `llm-api-temporal-ui` to `llm-api-temporal-ui`.
- Update the hardcoded secret names in the `deploy.yml` sync steps: `llm-api-secrets` → `llm-api-secrets`, `llm-api-db-secret` → `llm-api-db-secret`.
- After the first successful deploy to the new repos, delete the old `llm-api` and `llm-api-temporal-ui` ECR repos manually from the AWS console.
- No IAM changes needed — the existing OIDC role policy grants access to all ECR repos in the account.

### Temporal Task Queue

Renaming `llm-api-training` → `llm-api-training` orphans any in-flight Temporal workflows. This change must be applied when no workflows are running. The rename touches `worker.py`, `workflows.py`, `activities.py`, and `trigger_training.py`.

### Model Filename

`model.gguf` → `model.gguf` is updated in Python source and the k8s `MODEL_PATH` env var. Any GGUF already uploaded to S3 under the old key continues to work until the next training run uploads under the new key. No migration of existing S3 objects is required.
...
When the PR merges, `deploy.yml` applies the renamed manifests. Kubernetes creates the new `llm-api` resources alongside the existing `llm-api` ones. After rollout succeeds, run the following manual cleanup:
...
| Path | Action |
|------|--------|
| `infra/k8s/llm-api/` | Renamed from `infra/k8s/llm-api/` |
| `infra/k8s/llm-api/*.yaml` | Updated — all `llm-api*` / `llm-api-db*` resource names replaced |
...
| Python source (all remaining) | Modified — `model.gguf` → `model.gguf`, `~/llm-api` → `~/llm-api` |
...
- [ ] `kubectl delete` old `llm-api` resources (see k8s migration section)
- [ ] Delete old `llm-api` ECR repo from AWS console after first successful push to `llm-api`
