            ...
            "cells": [
                {
                    ...
                    "source": [
                        ...
                        "], check=True)\n",
                        "# Force Python to rescan sys.path directories after subprocess pip installs.\n",
                        "# Without this, importlib's cached directory listings miss newly installed packages.\n",
                        "import importlib; importlib.invalidate_caches()\n",
                        "# Pre-import the EXACT classes trainer.py needs, serving two purposes:\n",
                        "# 1. Catches import failures early with a diagnostic message (transformers uses\n",
                        "#    lazy loading — 'import transformers' always succeeds even when submodule\n",
                        "#    imports like AutoModelForCausalLM would fail due to dependency conflicts).\n",
                        "# 2. Warms up the lazy module so trainer.py's identical 'from transformers import'\n",
                        "#    block finds everything already cached in sys.modules and sets\n",
                        "#    _TRANSFORMERS_AVAILABLE=True reliably.\n",
                        "try:\n",
                        ...
                        "    print('transformers + datasets ready', flush=True)\n",
                        "except Exception as _e:\n",
                        "    raise RuntimeError(f'dep import failed after install: {_e}  sys.path={sys.path}') from _e\n",
                        "# Purge any stale domain/adapter modules left in sys.modules from a reused\n",
                        "# kernel process. Without this, trainer.py might be cached with\n",
                        "# _TRANSFORMERS_AVAILABLE=False even though transformers is now installed.\n",
                        "_stale = [k for k in sys.modules if k.startswith(('domain.', 'adapters.', 'interactors.'))]\n",
                        "for _k in _stale: del sys.modules[_k]\n",
                        "\n",
